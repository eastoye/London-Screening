...
export function discoverImaxProgrammeCards(html: string): { cards: ProgrammeCard[]; errors: string[] } {
  const discovered = discoverProgrammeCards(html);
  return {
    cards: discovered.cards.filter((card) => /BFI IMAX|IMAX, Waterloo|\bIMAX\b/i.test(card.body)),
...
function explicitValueLabels(value: unknown): string[] {
  if (typeof value === "string") return cleanText(value) ? [cleanText(value)] : [];
  if (Array.isArray(value)) return unique(value.flatMap(explicitValueLabels));
  if (value && typeof value === "object") {
    const object = value as Record<string, unknown>;
    const label = cleanText(object.name) || cleanText(object.value);
    return label ? [label] : [];
  }
  return [];
}
...
function explicitLabels(listingLabel: string, calendar: CalendarPerformance | null): string[] {
  ...
  if (calendar) {
    labels.push(...explicitValueLabels(calendar.format), ...explicitValueLabels(calendar.formats));
    labels.push(...explicitValueLabels(calendar.extras));
...
function projectionFormats(labels: string[]): ProjectionFormat[] {
  ...
  if (!result.includes("imax")) result.unshift("imax");
  return result;
}
...
export function publicBookingUrl(rawBookingUrl: string | null): string | null {
  ...
  try {
    ...
    if (/^\/(?:imax\/)?Online\/(?:mapSelect|seatSelect)\.asp$/i.test(source.pathname)) {
      return performanceUuid(source.href) ? source.href : null;
    }
    if (!/^\/(?:imax\/)?Online\/login\.asp$/i.test(source.pathname)) return null;
    const target = source.searchParams.get("targetPage");
    if (!target) return null;
    const decodedTarget = decodeURIComponent(target);
    if (!/^(?:mapSelect|seatSelect)\.asp\?/i.test(decodedTarget)) return null;
    const prefix = /^\/imax\/Online\//i.test(source.pathname) ? "/imax/Online" : "/Online";
    const destination = new URL(`${prefix}/${decodedTarget}`, TICKET_ORIGIN);
...
function safeFilmTitleHint(card: ProgrammeCard, detail: DetailData): string | null {
  ...
  const stripped = card.title
    ...
    .replace(/\s+-\s+(?:\d+(?:st|nd|rd|th) anniversary(?: screening)?(?: IMAX exclusive)?|IMAX exclusive previews?|restoration (?:world |UK )?(?:IMAX )?premiere|(?:world|UK) (?:IMAX )?premiere|preview)\s*$/i, "")
    .trim();
  return stripped || null;
}

function fallbackReference(card: ProgrammeCard, startTimeIso: string, screenName: string | null): string {
  return `${card.title}|${startTimeIso}|${screenName ?? "imax"}`
    .toLowerCase().replace(/[^a-z0-9|:-]+/g, "-");
}

export function parseBfiImax(
  programmeHtml: string,
  detailPages: ReadonlyMap<string, string>,
  now: Date,
): ParseResult {
  const discovered = discoverImaxProgrammeCards(programmeHtml);
  ...
  for (const card of discovered.cards) {
    ...
    const imaxRows = detail.calendar.filter((calendar) => cleanText(calendar.venue) === "BFI IMAX, Waterloo");
    const structuredTimes = new Set<string>();
    sourceCount += imaxRows.length;
    for (const calendar of imaxRows) {
      const parsedTime = typeof calendar.datetime === "string" ? new Date(calendar.datetime) : new Date(NaN);
      if (!Number.isFinite(parsedTime.getTime())) {
        errors.push(`${card.title}: structured IMAX performance had an invalid datetime.`);
        continue;
      }
      if (parsedTime <= now) continue;
      const startTimeIso = parsedTime.toISOString();
      structuredTimes.add(startTimeIso);
      const rawBooking = typeof calendar.bookingUrl === "string" ? safeUrl(calendar.bookingUrl, TICKET_ORIGIN) : null;
      const uuid = performanceUuid(rawBooking);
      const soldOut = calendar.soldOut === true;
      const screenName = cleanText(calendar.screen).replace(/^Screen\s+/i, "") || "BFI IMAX, Waterloo";
      const labels = explicitLabels("", calendar);
      const bookingUrl = soldOut ? null : publicBookingUrl(rawBooking);
      const openForSale = calendar.onSale === true && calendar.available === true;
      ...
      screenings.push({
        ...
        startTimeIso,
        bookingUrl,
        sourceReference: `bfi-imax:${uuid ?? fallbackReference(card, startTimeIso, screenName)}`,
    ...
    const listed = parseImaxListingPerformances(card, now);
    errors.push(...listed.errors);
    for (const row of listed.rows) {
      if (structuredTimes.has(row.startTimeIso)) continue;
      sourceCount++;
      const uuid = performanceUuid(row.rawBookingUrl);
      const bookingUrl = row.soldOut ? null : publicBookingUrl(row.rawBookingUrl);
      const labels = explicitLabels(row.label, null);
      const label = labels.map((value) => value.replace(/\bSold out\b/gi, "").replace(/\s+/g, " ").trim())
        .filter(Boolean).join(" · ") || null;
      screenings.push({
        movieTitle: card.title,
        ...
        bookingUrl,
        sourceReference: `bfi-imax:${uuid ?? fallbackReference(card, row.startTimeIso, row.screenName)}`,
        sourceReleaseYear: card.releaseYear,
        sourceRuntimeMinutes: card.runtimeMinutes,
        sourceDirectors: detail.directors,
        sourceCountries: detail.countries,
        ...
        accessibilityFeatures: accessibilityFeatures(labels),
        programmeTypes: programmeTypes(labels),
        availabilityStatus: row.soldOut ? "sold_out" : bookingUrl ? "available" : "unknown",
        screenName: row.screenName ?? "BFI IMAX, Waterloo",
        screeningLabel: label,
        screeningTags: screeningTags(labels),
        soldOut: row.soldOut,
      });
