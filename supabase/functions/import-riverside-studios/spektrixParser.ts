...
export function buildScreening(
  ...
  return {
    ...
    event_thumbnail_url: event.thumbnailUrl,
    event_attributes: event.attributes,
    instance_is_on_sale: instance.isOnSale,
...
export async function fetchAllScreenings(
  ...
  for (const event of cinemaEvents) {
    try {
      ...
      if (!instances.length && options?.fromDate) {
        const last = event.lastInstanceDateTime ? new Date(event.lastInstanceDateTime) : null;
        if (last && Number.isFinite(last.getTime()) && last.getTime() >= options.fromDate.getTime()) {
          throw new Error("future-dated event returned zero instances");
        }
      }
      ...
      const last = event.lastInstanceDateTime ? new Date(event.lastInstanceDateTime) : null;
      const futureRelevant =
        !options?.fromDate ||
        Boolean(last && Number.isFinite(last.getTime()) && last.getTime() >= options.fromDate.getTime());
      if (futureRelevant) errors.push(msg);
      console.warn(`[spektrix] ${msg}`);
    }
  }

  if (errors.length) {
    throw new Error(`Riverside Spektrix source was incomplete: ${errors.slice(0, 5).join("; ")}`);
  }
