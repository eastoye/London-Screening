import {cardMetadata,performanceMetadata,enrichDetails} from "../_shared/bfiMetadata.ts";
...
export type Row = ReturnType<typeof cardMetadata> & ReturnType<typeof performanceMetadata> & {source_directors?:string[];source_countries?:string[]} & { cinema_name:string; movie_title:string; start_time:string; booking_url:string|null; format:string|null; sold_out:boolean; projection_formats:ProjectionFormat[]; availability_status:AvailabilityStatus; source_reference:string; last_seen_at:string; active?:boolean };
...
export async function fetchBfiRows(venue:Venue,now:Date):Promise<Row[]>{
  ...
  const responses=await Promise.all(paths.map(path=>fetch(SOURCE+path,{signal:AbortSignal.timeout(20000),headers:{"User-Agent":"Cinema Listings importer/1.0","Accept":"text/html"}})));
  ...
  for(const card of html.split(/<article\b[^>]*class="[^"]*showCard[^"]*"[^>]*>/i).slice(1)){
    const end=card.indexOf("</article>"); if(end<0)throw new Error("Truncated BFI card"); const body=card.slice(0,end);
    const title=clean(body.match(/<h3\b[^>]*>[\s\S]*?<a\b[^>]*class="card-link"[^>]*>([\s\S]*?)<\/a>/i)?.[1]||"");
    if(!title)throw new Error("BFI card missing title");
    ...
    while((section=sectionRe.exec(body))){
      ...
      for(const rawLi of section[2].split(/<li\b[^>]*>/i).slice(1)){
        ...
        const start=parseDateTime(label,now);if(!start)throw new Error("Unparseable BFI performance date");if(new Date(start)<=now)continue;
        ...
        const rawBookingUrl=link?decode(link[1]):null;
        // BFI's login redirect is unsuitable for anonymous browsing. The public
        // event page exposes its performance booking choices without a login.
        const bookingUrl=rawBookingUrl&&/\/login\.asp(?:\?|$)/i.test(rawBookingUrl)
          ?cardMetadata(body).source_event_url:rawBookingUrl;
        const sourceFormats=normaliseProjectionFormats([tagText]);
        const projectionFormats=venue==="imax"
          ? (["imax", ...sourceFormats.filter(value=>value!=="imax")] as ProjectionFormat[])
          : sourceFormats;
        rows.push({cinema_name:cinema,movie_title:title,start_time:start,booking_url:bookingUrl,format:formats(tagText,venue),sold_out:sold,projection_formats:projectionFormats,availability_status:availabilityFromSignals({soldOut:sold,openForSale:link?true:null,hasBookingUrl:Boolean(bookingUrl)}),source_reference:`${prefix}:${uuid||fallback}`,last_seen_at:now.toISOString(),...cardMetadata(body),...performanceMetadata(tagText,screen)});
      }
    }
    if(parsedLinks!==[...body.matchAll(/data-ga-event="booking_click"/g)].length)throw new Error("Incomplete BFI performance parsing");
  }
  ...
  const result=[...byReference.values()];await enrichDetails(result);return result;
}
...
export async function runImporter(req:Request,venue:Venue,min:number){
  ...
  try{
    ...
    const now=new Date(),{count:previous,error:countError}=await db.from("screenings").select("id",{count:"exact",head:true}).eq("cinema_name",cinema).eq("active",true).gt("start_time",now.toISOString());
