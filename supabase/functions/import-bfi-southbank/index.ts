...
import { discoverProgrammeCards, parseBfiSouthbank } from "./parser.ts";

const PROGRAMME_URL = "https://cinemas.bfi.org.uk/whats-on";
const CINEMA_NAME = "BFI Southbank";
const MIN_SCREENINGS = 20;
const RATIO_GUARD_MIN_EXISTING = 20;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const discovery = discoverProgrammeCards(programmeHtml);
    ...
    const parsed = parseBfiSouthbank(programmeHtml, detailPages, startedAt);
    ...
    return jsonResponse({
      ...
      metadata: {
        ...
        stable_performance_ids: records.filter((row) => /^bfi-southbank:[0-9A-F-]{36}$/.test(row.source_reference)).length,
      },
