...
import { CINEMA_NAME, parsePeckhamplex } from "./parser.ts";
...
Deno.serve(async (request: Request) => {
  ...
  let found = 0;
  try {
    const parsed = await parsePeckhamplex(startedAt);
    const records = parsed.records;
    found = records.length;
    const duplicateReferences = duplicateKeys(records, (record) => record.source_reference);
    if (duplicateReferences.length) throw new Error(`Duplicate Peckhamplex references: ${duplicateReferences.slice(0, 5).join(", ")}`);
    const duplicateTitleTimes = duplicateKeys(records, (record) => `${record.movie_title}\u0000${record.start_time}`);
    if (duplicateTitleTimes.length) throw new Error(`Duplicate Peckhamplex title/time rows (${duplicateTitleTimes.length})`);
    ...
    if (previousError) throw new Error(`Could not read previous Peckhamplex count: ${previousError.message}`);
    if ((previous ?? 0) >= 10 && records.length < Math.ceil((previous ?? 0) * 0.5)) {
      throw new Error(`Suspicious Peckhamplex count drop from ${previous} to ${records.length}`);
    }
    ...
    return jsonResponse({
      success: true,
      cinema: CINEMA_NAME,
      source: "official-peckhamplex-and-veezi",
      screenings_found: found,
      screenings_saved: committed.saved,
      previous_active: previous ?? 0,
      schedule_screenings: parsed.scheduleScreenings,
      supplemental_screenings: parsed.supplementalScreenings,
