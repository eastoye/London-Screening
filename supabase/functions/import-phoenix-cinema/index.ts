...
const PROGRAMME_URL = "https://www.phoenixcinema.co.uk/PhoenixCinemaLondon.dll/WhatsOn";
...
async function fetchProgramme(): Promise<string> {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Phoenix programme returned HTTP ${response.status}.`);
    const html = await response.text();
    if (html.length < 10_000) {
      throw new Error(`Phoenix programme was unexpectedly small (${html.length} bytes).`);
    }
    return html;
  } finally {
...
async function previousActiveCount(ctx: ImportRunContext, now: Date): Promise<number> {
  ...
  if (error) throw new Error(`Could not read previous Phoenix count: ${error.message}`);
  return count ?? 0;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (parsed.errors.length) {
      throw new Error(`Phoenix source was incomplete: ${parsed.errors.slice(0, 8).join(" | ")}`);
    }
    const validationErrors = validateScreenings(parsed.screenings);
    if (validationErrors.length) {
      throw new Error(`Phoenix validation failed: ${validationErrors.slice(0, 8).join(" | ")}`);
    }
    if (found < MIN_SCREENINGS) {
      throw new Error(`Unusually low Phoenix screening count (${found}); database left untouched.`);
    }
