...
import { parseKilnProgramme } from "./parser.ts";
...
Deno.serve(async (request: Request) => {
  ...
  if (!started.runId) {
    return jsonResponse({ success: false, error: started.error ?? "Could not start import" }, 500);
  }

  let found = 0;
  try {
    const now = new Date();
    const programme = await parseKilnProgramme(now);
    const records = programme.records;
    found = records.length;

    if (records.length < MIN_SCREENINGS) {
      throw new Error(`Unusually low Kiln screening count (${records.length})`);
    }

    const duplicateReferences = duplicates(records, (record) => record.source_reference);
    if (duplicateReferences.length) {
      throw new Error(`Duplicate Kiln references: ${duplicateReferences.slice(0, 5).join(", ")}`);
    }
    const duplicateTitleTimes = duplicates(records, (record) => `${record.movie_title}\u0000${record.start_time}`);
    if (duplicateTitleTimes.length) {
      throw new Error(`Duplicate Kiln title/time rows (${duplicateTitleTimes.length})`);
    }
    ...
    if (countError) throw new Error(`Could not read previous Kiln count: ${countError.message}`);
    if ((previous ?? 0) >= 10 && records.length < Math.ceil((previous ?? 0) * 0.5)) {
      throw new Error(`Suspicious Kiln count drop from ${previous} to ${records.length}`);
    }
