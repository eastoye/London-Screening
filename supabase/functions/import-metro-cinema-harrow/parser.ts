...
export async function parseMetroScreenings(now: Date): Promise<ParsedProgramme> {
  ...
  for (const movie of programme.movies) {
    ...
    for (const show of movie.show_times ?? []) {
      ...
      const reference = `metro-cinema:showtime:${identifier}`;
  ...
  if (parseErrors.length) throw new Error(`Metro programme parsing incomplete: ${parseErrors.slice(0, 5).join("; ")}`);
  return {
