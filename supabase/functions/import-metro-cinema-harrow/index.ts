...
import { MIN_SCREENINGS, parseMetroScreenings } from "./parser.ts";
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const now = new Date();
    const programme = await parseMetroScreenings(now);
    const records = programme.records;
    found = records.length;
    if (records.length < MIN_SCREENINGS) throw new Error(`Unusually low Metro screening count (${records.length})`);
    const duplicateReferences = duplicates(records, (record) => record.source_reference);
    if (duplicateReferences.length) throw new Error(`Duplicate Metro source references: ${duplicateReferences.slice(0, 5).join(", ")}`);
    const duplicateTitleTimes = duplicates(records, (record) => `${record.movie_title}\u0000${record.start_time}`);
    if (duplicateTitleTimes.length) throw new Error(`Duplicate Metro title/time rows (${duplicateTitleTimes.length})`);
    ...
    if (countError) throw new Error(`Could not read previous Metro count: ${countError.message}`);
    if ((previous ?? 0) >= 10 && records.length < Math.ceil((previous ?? 0) * 0.5)) {
      throw new Error(`Suspicious Metro count drop from ${previous} to ${records.length}`);
    }
