...
export const CINEMA_NAME = "Lumiere Romford";
export const SITE_ORIGIN = "https://www.lumiereromford.com";
const LOCATION_ID = "4";
...
async function cineSyncRequest(data: unknown): Promise<Record<string, unknown>> {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (!response.ok) throw new Error(`Lumiere CineSync HTTP ${response.status}`);
      const payload = await response.json();
      if (payload?.status !== true || !payload?.data || typeof payload.data !== "object") {
        throw new Error(`Lumiere CineSync returned an invalid response: ${JSON.stringify(payload).slice(0, 500)}`);
      }
...
async function moviesForDate(sessionDate: string): Promise<CineSyncMovie[]> {
  ...
  do {
    ...
    if (!Array.isArray(data.movies)) {
      throw new Error(`Lumiere CineSync list was incomplete for ${sessionDate}`);
    }
    ...
    if (totalPages > 10) throw new Error(`Unexpected Lumiere pagination (${totalPages} pages)`);
    page++;
  } while (page <= totalPages);
  if (movies.length === 0) throw new Error(`Lumiere CineSync returned no catalogue entries for ${sessionDate}`);
  return movies;
}
...
export async function fetchProgramme(): Promise<ProgrammeResult> {
  ...
  if (dates.length === 0 || dates.length > 120) {
    throw new Error(`Unexpected Lumiere programme date count (${dates.length})`);
  }

  const movies: CineSyncMovie[] = [];
  for (let index = 0; index < dates.length; index += 8) {
    const batch = await Promise.all(dates.slice(index, index + 8).map(moviesForDate));
...
export function bookingUrl(movie: CineSyncMovie, show: CineSyncShowtime): string | null {
  if (show.is_this_sold_by_third_party_system === "1") return absoluteUrl(show.third_party_system_url);
