...
import { MIN_SCREENINGS, parseLumiereScreenings } from "./parser.ts";
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const now = new Date();
    const programme = await parseLumiereScreenings(now);
    const records = programme.records;
    found = records.length;
    if (records.length < MIN_SCREENINGS) throw new Error(`Unusually low Lumiere screening count (${records.length})`);
    const duplicateReferences = duplicates(records, (record) => record.source_reference);
    if (duplicateReferences.length) throw new Error(`Duplicate Lumiere source references: ${duplicateReferences.slice(0, 5).join(", ")}`);
    const duplicateTitleTimes = duplicates(records, (record) => `${record.movie_title}\u0000${record.start_time}`);
    if (duplicateTitleTimes.length) throw new Error(`Duplicate Lumiere title/time rows (${duplicateTitleTimes.length})`);
    ...
    if (countError) throw new Error(`Could not read previous Lumiere count: ${countError.message}`);
    if ((previous ?? 0) >= 15 && records.length < Math.ceil((previous ?? 0) * 0.5)) {
      throw new Error(`Suspicious Lumiere count drop from ${previous} to ${records.length}`);
    }
    ...
    return jsonResponse({
      ...
      excluded_by_reason: programme.excludedByReason,
